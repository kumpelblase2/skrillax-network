//! This module underpins the security aspects of the network communication
//! between a Silkroad Online client and server. It provides the building blocks
//! to establish a common encryption secret ([SilkroadEncryption]) through a
//! handshake ([handshake::ActiveHandshake], [handshake::PassiveHandshake]),
//! which als provide CRC and count security checks. This provided without any
//! I/O, such that you could feasibly add it to whatever setup you have, or you
//! can use the [skrillax-stream](https://docs.rs/skrillax-stream) crate that provides an async version on
//! top of tokio (albeit with a bit more than just the handshake).
//!
//! The main way you want to be interfacing with this crate is through the
//! handshake structs of which there are two: [handshake::ActiveHandshake] &
//! [handshake::PassiveHandshake]. As the name suggests, the handshake is
//! composed of an active and a passive part, usually being the server
//! and the client respectively. The server is considered the active part of the
//! handshake, because it will start the handshake procedure by sending the
//! passive part (i.e. the client) initialization data. After which a few more
//! messages will be exchanged until both parties have both setup their security
//! bytes ([MessageCounter] & [Checksum]) as well as the shared encryption
//! ([SilkroadEncryption]). Please refer to the documentation of the individual
//! handshake element for more information about the procedure of a handshake.
//!
//! Each of the three elements ([MessageCounter], [Checksum], and
//! [SilkroadEncryption]) can be used independently of each other if so desired.
...
pub enum SilkroadSecurityError {
    /// The handshake hasn't been started or hasn't completed, yet the operation
    /// required it.
    #[error("Security has not been initialized")]
    SecurityUninitialized,
    /// The handshake has already completed. The security would need to be reset
    /// before continuing.
    #[error("Security is already initialized")]
    AlreadyInitialized,
    /// Finalizing the handshake requires the handshake to have exchanged public
    /// key data, which hasn't happened yet.
    #[error("Security has not completed the initialization")]
    InitializationUnfinished,
    /// The given encrypted data is not the correct block length, as required
    /// for decryption.
...
///
/// Generally only the client encrypts data, but this is a generic encryption
/// setup. This is essentially a thin veil around [BlowfishLE], only ensuring
/// the right block size has been used.
///
...
impl SilkroadEncryption {
    ...
    ///
    /// Decrypts the given input by splitting it into the individual encrypted
    /// blocks. The output is all decrypted data, which may contain padding
    /// that was added before encryption. Bytes are copied before performing
    /// decryption. To decrypt in place, use
    /// [decrypt_mut][Self::decrypt_mut()].
    ///
    ...
    ///
    /// Decrypts the given input by splitting it into the individual encrypted
    /// blocks in place. The decrypted data may still be padded to match
    ...
    ///
    /// Encrypts the given bytes using the previously established secret.
    /// Requires that the handshake has been completed. It will copy the
    /// bytes and return the encrypted bytes as an owned reference. Bytes will
    ...
    ///
    /// Encrypts the given bytes using the previously established secret.
    /// Requires that the handshake has been completed and that `data` is a
