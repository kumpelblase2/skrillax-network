...
macro_rules! define_input_protocol {
    ($name:ident => $($enumValue:ident),* + $($innerProto:ident),*) => {
        impl $crate::__internal::InputProtocol for $name {
            ...
            fn create_from(opcode: u16, data: &[u8], ctx: skrillax_packet::SerdeContext) -> Result<(usize, Box<Self>), $crate::__internal::InStreamError> {
                match opcode {
                    $(
...
macro_rules! define_output_protocol {
    ($name:ident => $($enumValue:ident),* + $($innerProto:ident),*) => {
        impl $crate::__internal::AsPacket for $name {
            fn as_packet(&self, ctx: &skrillax_packet::SerdeContext) -> $crate::__internal::OutgoingPacket {
                match self {
                    $(
