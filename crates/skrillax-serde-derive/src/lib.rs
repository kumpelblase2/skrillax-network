//! Generally it should be enough to simply `#[derive(Deserialize)]` or whichever trait you need.
//! Just like the more general `serde` crate, this will handle most common things, like fields of
...
//! provide additional options through the `#[silkroad]` tag. Which options are available for
...
//! specify using `#[silkroad(size = 2)]` on the enum itself:
//! ```
//! #[derive(Serialize, Deserialize)]
//! #[silkroad(size = 2)]
//! enum Hello {
...
//! ## Structs
//!
//! Structs are always serialized/deserialized by serializing/deserializing their fields. A unit
//! struct therefor has length zero. There are also no options currently to alter the behavior for
//! structs themselves, only their fields.
//!
//! ```
//! #[derive(Serialize, Deserialize)]
//! struct Hello(String);
...
//! ## Fields
//!
//! The serialization/deserialization of fields is identical between structs and enums. Each field
//! is serialized one after another without any separators. Therefor, it is necessary to match the
//! size exactly to the consumed bytes. Fields are serialized and deserialized in the order they
//! are defined.
//!
//! ```
//! #[derive(Serialize, Deserialize)]
...
//! attribute. This accepts one of three options: `length` (default), `break`, and `has-more`.
//! `break` and `has-more` specify before each element if another element will follow using
//! different values. `break` uses `1` for 'has more values' and `2` for finished, while `has-more`
...
//! can be configured by using a `size` of 2.
//! ```
//! #[derive(Serialize, Deserialize)]
//! struct Hello {
//!     #[silkroad(size = 2)]
...
//! ## Optional
//!
//! Optional values will be encoded using a byte denoting the presence (1) or absence (0), following
//! the underlying value if it is present. In some cases, due to previous knowledge, optional values
//! may just appear (or be missing) without the presence indicator. This makes them impossible to
//! deserialize (currently), but this is unfortunately current necessary. To achieve this, you can
//! set the size of the field to 0.
//! ```
//! #[derive(Serialize)]
//! struct Hello {
//!     #[silkroad(size = 0)]
